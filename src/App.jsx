...
// Первые 4 достижения из 16 — остальные добавим отдельно позже.
const ACHIEVEMENTS=[
  {id:"newbie",title:"Новичок",tier:"НАЧАЛО ПУТИ",desc:"Послушайте первый трек",
    unlocked:u=>(u.totalListenSec||0)>0, progress:u=>(u.totalListenSec||0)>0?1:0,
    progressLabel:u=>(u.totalListenSec||0)>0?"Готово":"0/1"},
  {id:"melon",title:"Меломан",tier:"ПРОСЛУШИВАНИЕ",desc:"Прослушайте 50 часов суммарно",target:50*3600,
    unlocked:u=>(u.totalListenSec||0)>=50*3600, progress:u=>Math.min(1,(u.totalListenSec||0)/(50*3600)),
    progressLabel:u=>`${Math.floor((u.totalListenSec||0)/3600)}/50 ч`},
  {id:"audiophile",title:"Аудиофил",tier:"ПРОСЛУШИВАНИЕ",desc:"Прослушайте 500 часов суммарно",target:500*3600,
    unlocked:u=>(u.totalListenSec||0)>=500*3600, progress:u=>Math.min(1,(u.totalListenSec||0)/(500*3600)),
    progressLabel:u=>`${Math.floor((u.totalListenSec||0)/3600)}/500 ч`},
  {id:"collector",title:"Коллекционер",tier:"ОФОРМЛЕНИЕ",desc:"Закрепите трек в профиле",
    unlocked:u=>!!u.pinnedTrack, progress:u=>u.pinnedTrack?1:0,
    progressLabel:u=>u.pinnedTrack?"Готово":"0/1"},
];
function ProfileView({uid,myUid,onClose,onStartChat,onEditProfile}){
      ...
      <div style={{position:"relative",minHeight:286,padding:"calc(max(env(safe-area-inset-top,24px),24px) + 72px) 18px 20px",overflow:"hidden",
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",textAlign:"center"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,marginTop:18}}>
            ...
            {isMe&&<button onClick={onEditProfile} style={{background:"rgba(255,255,255,.12)",border:"none",width:30,height:30,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><IcPencilSm size={14} color="#fff"/></button>}
          </div>
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:5,marginTop:14}}>
            ...
            {user.createdAt&&(
              <div style={{display:"flex",alignItems:"center",gap:6,color:"rgba(255,255,255,.45)",fontSize:12.5}}>
                <IcCalendarSm size={13} color="rgba(255,255,255,.45)"/>
                В Mimose с {fmtJoinedDate(user.createdAt?.toDate?user.createdAt.toDate().getTime():(user.createdAt?.seconds?user.createdAt.seconds*1000:0))}
              </div>
            )}
          </div>
          ...
          {unlockedAchievements.length>0&&(
            <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:8,marginTop:16}}>
              {unlockedAchievements.map(a=>(
                <div key={a.id} style={{padding:"9px 16px",borderRadius:20,background:"rgba(255,255,255,.09)",border:"1px solid rgba(255,255,255,.1)",color:"#fff",fontSize:13,fontWeight:700}}>{a.title}</div>
              ))}
            </div>
          )}
        </div>
      ...
      <div style={{padding:"14px 14px max(env(safe-area-inset-bottom,18px),18px)"}} onClick={()=>setShowMenu(false)}>
        ...
        {user.pinnedTrack&&(
          <div style={{background:profileSurface,borderRadius:16,padding:"12px 14px",marginBottom:12,animation:"fadeUp 0.4s ease 0.09s both"}}>
            ...
            <button onClick={()=>audio.playTrack(user.pinnedTrack)} style={{display:"flex",alignItems:"center",gap:12,width:"100%",background:"transparent",border:"none",padding:0,cursor:"pointer",fontFamily:"inherit",textAlign:"left"}}>
              ...
              <div style={{width:36,height:36,borderRadius:"50%",background:surface2,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><IcAudioPlay size={16}/></div>
            </button>
          </div>
        )}
        ...
        <div style={{background:profileSurface,borderRadius:16,padding:"14px 16px",marginBottom:12,animation:"fadeUp 0.4s ease 0.11s both"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {ACHIEVEMENTS.map(a=>{
              ...
              return(
                <div key={a.id} style={{background:done?surface2:"transparent",border:`1px solid ${done?border:"transparent"}`,borderRadius:14,padding:"12px 14px",opacity:done?1:0.75}}>
                  <div style={{display:"flex",alignItems:"flex-start",gap:12}}>
                    <div style={{width:40,height:40,borderRadius:"50%",background:done?`linear-gradient(135deg,${profileAccent},${accent2})`:surface2,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      {done&&<div style={{color:"#4CAF50",fontSize:12,fontWeight:700,marginTop:4,display:"flex",alignItems:"center",gap:5}}><IcCheckOne size={13} color="#4CAF50"/>Надето</div>}
                    </div>
                  </div>
                </div>
              );
      ...
      <div style={{height:"100vh",position:"relative",overflow:"hidden",animation:"fadeIn 0.4s ease both"}}>
        ...
        {viewProfileUid&&(
          <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,zIndex:600}}>
            <ProfileView uid={viewProfileUid} myUid={fbUser?.uid} onClose={()=>setViewProfileUid(null)} onStartChat={startChatWithUser} onEditProfile={()=>{setViewProfileUid(null);setEditing(true);}}/>
          </div>
        )}
